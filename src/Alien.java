...
public class Alien extends GameObject {
	...
	void update() {
		if(randomDirection==0 && row > 0 && (ObjectManager.tiles[row-1][column].tileType==0 || ObjectManager.tiles[row-1][column].tileType==3)) {
			row--;
		}
		else if(randomDirection==0 && row > 0 && ObjectManager.tiles[row-1][column].tileType==1) {
			changeDirection();
		}
		if(randomDirection==1 && row < ObjectManager.tiles.length-1 && ObjectManager.tiles[row+1][column].tileType==0) {
			row++;
	    }
		else if(randomDirection==1 && row < ObjectManager.tiles.length-1 && ObjectManager.tiles[row+1][column].tileType==1) {
			changeDirection();
		}
		if(randomDirection==2 && column > 0 && ObjectManager.tiles[row][column-1].tileType==0) {
			column--;
	    }
		else if(randomDirection==2 && column > 0 && ObjectManager.tiles[row][column-1].tileType==1) {
			changeDirection();
		}
		if(randomDirection==3 && column < ObjectManager.tiles[row].length-1 && ObjectManager.tiles[row][column+1].tileType==0) {
			column++;
	    }
